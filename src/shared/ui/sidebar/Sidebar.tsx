...
const Sidebar: React.FC<SidebarProps> = ({ orientation, items, onSelectItem }) => {
    ...
    <div className="flex">
      <div
        className={clsx(
          ...
          orientation === 'vertical' ? 'w-[224px] flex-col' : 'h-[54px] w-auto flex-row',
        )}
      >
        {items.map((item, index) => (
          <Link
            ...
            className={clsx(
              ...
              selectedItem === item
                ? 'bg-opacity-[8%]'
                : 'bg-opacity-[0%] hover:bg-opacity-[4%] animation',
            )}
          >
